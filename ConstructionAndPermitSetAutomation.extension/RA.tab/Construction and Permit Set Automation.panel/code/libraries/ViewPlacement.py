...
# CONSTANTS
GRID_RESOLUTION = 30
PROCESSED_SHEETS_LIMIT = 15
DETAIL_VIEWS_LIMIT_PER_SHEET = 10
...
def PlaceViewsOnSheets():
    ...
    # Main process
    iterator = 0
    limit = PROCESSED_SHEETS_LIMIT
    for sheet in sheets:

        # Collecting data from the sheet number of the current sheet to then compare to specific data in each view
        fullSheetNumber = sheet.SheetNumber # AP4.01
        sheetNumberSymbol = fullSheetNumber.split(".")[0] # AP4
        sheetNumberNumber = fullSheetNumber.split(".")[1] # 01
        ...
        for view in views:
            ...
            if view.UniqueId in placedViewsUniqueIDs:
                continue
        ...
        if viewsWithSimilarityScore:
            ...
            # Specifies the amount of views that can be included in a sheet based on its drawing category
            if sheetDrawingTitleParameter == "floor plans":
                ...
                limitOfViews = DETAIL_VIEWS_LIMIT_PER_SHEET
            ...
            for view in viewsWithSimilarityScore:
                ...
                if not workspaceCells.cellList:
                    continue
                
                selectedView = view[0]
                print("Selected View: {}".format(selectedView.Name))
                ...
                placedViewsUniqueIDs.append(selectedView.UniqueId)
...
class Workspace():
    ...
    def CheckForAvailableCells(self, requestedSize):

        locationCell = None

        amountOfRequestedCells = list(map(lambda x : math.ceil(x / self.cellList[0].Size), requestedSize))
        amountOfRequestedCells = amountOfRequestedCells[0] * amountOfRequestedCells[1]

        viewportHeight = requestedSize[1]
        viewportWidth = requestedSize[0]
        
        # First filter
        filteredCells = list(filter(lambda x : x.SpaceAtBottom >= (viewportHeight / 2) and \
                                               x.SpaceAtTop >= (viewportHeight / 2) and \
                                               x.SpaceAtLeft >= (viewportWidth / 2) and \
                                               x.SpaceAtRight >= (viewportWidth / 2), self.cellList))

        viewportMaxMin = XYZ(viewportWidth / 2, viewportHeight / 2, 0)

        for cell in filteredCells:

            maxCoordinate = cell.Location + viewportMaxMin
            minCoordinate = cell.Location - viewportMaxMin

            areaCells = list(filter(lambda x : x.Location.X >= minCoordinate.X and \
                                               x.Location.Y >= minCoordinate.Y and \
                                               x.Location.X <= maxCoordinate.X and \
                                               x.Location.Y <= maxCoordinate.Y, self.cellList))

            amountOfCells = len(areaCells)

            # print(amountOfCells, amountOfRequestedCells)

            if amountOfCells >= amountOfRequestedCells:
                locationCell = cell

                # Filters out the cells that were occupied in this iteration
                self.cellList = [x for x in self.cellList if x not in areaCells]
